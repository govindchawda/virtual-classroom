...
export default function SearchClasses() {
    ...
    const [showClass, setShowClasses] = useState([]);
    const [manageClass, setManageClasses] = useState(true);
    // EDIT CLASSES STATES
    ...
    // SHOW AND HIDE INPUTES
    const inputValue = (value) => { 
        setSearchState(searchState == value ? null : value);   
    }

    // SEARCH CLASSES BY MULTIPLE ITEMS
    const searchByTittle = async (event) => {
        try {
            ...
            if(showTeacherOption.role == "admin"){
                var res = await axios.get(`${api}/classes/getall`, {
                    headers: {
            ...
            if(showTeacherOption.role == "teacher"){
                var res = await axios.get(`${api}/classes/showClassIncludeTeachers/${showTeacherOption._id}`, {
                    headers: {
                        "Authorization": "bearer " + localStorage.getItem("tokeId")
                    }
                });
            }
            const classes = res.data.result.filter((items, index) => (
                items.tittle.toLowerCase().includes(value.toLowerCase()) || items.classCode.toLowerCase().includes(value.toLowerCase())
                || items.teacherId.toLowerCase().includes(value.toLowerCase())
                || items.schedule[0].startTime.toLowerCase().includes(value.toLowerCase()) || items.schedule[0].endTime.toLowerCase().includes(value.toLowerCase())
            ));
            setShowClasses(classes);
        } catch (error) {
            console.log("search by tittle", error);
        }
    }

    // SHOW SEARCH CLASSES
    const viewclass = () => {
        setManageClasses(manageClass == false ? true : false);
    }
        ...
        try {
            ...
            toast(res.data.message);
             if(res.data.success ===  true && res.data.result?.students){
                 setUpdateClass(prev => ({...prev, students: res.data.result.students}));
                setADDStudents('');
                toast(res.data.message);
                } else{
            ...
            <div className="container-fluid">
                <div className="container">
                    ...
                    <div className='mt-2'>
                        ...
                        <div className='mt-4'>
                            {searchState == 'tittle' ? <input type="search" placeholder='search by Class Tittle' onChange={searchByTittle} className='form-control ' /> : ''}
                            {searchState == 'classId' ? <input type="search" placeholder='search by classCode' onChange={searchByTittle} className='form-control ' /> : ''}
                            {searchState == 'TeacherId' ? <input type="search" placeholder='search by teacherID' onChange={searchByTittle} className='form-control ' /> : ''}
                            {searchState == 'time' ? (
                                <>
                                    <div className='d-flex justify-content-between'>
                                        <div className="col-sm-5">
                                            <input type="search" placeholder='search by Class Start Time 10:00am' onChange={searchByTittle} className='form-control' />
                                        </div>
                                        <div className="col-sm-5">
                                            <input type="search" placeholder='Search by Class End Time 7:00pm' onChange={searchByTittle} className='form-control ' />
                                        </div>
                                    </div>
                                </>
                            )
                        ...
                        <div className="mt-4 d-flex">
                            <div className="col-sm-12 dflex flex-wrap">
                                            ...
                                            <div className='col-sm-4 p-2 ' key={index}>
                                                <div className="col-sm-12 color pb-2">
                                                    <div className="col-sm-12  tittle color">
                                                        ...
                                                        <div className='class-info d-flex justify-content-between'>
                                                            <p><b>Day : </b> {items.schedule[0].day.join(",")}</p>
                                                        </div>
                                                        <div className='class-info d-flex justify-content-between'>
                                                    ...
                                                    <div className="d-flex justify-content-between px-3">
                                                        <button onClick={() => editClasses(items._id)} className="btn btn-primary" data-bs-toggle="modal" data-bs-target="#staticBackdrop">Edit</button>
                                                        <button onClick={() => deleteClasses(items._id)} className='btn btn-danger'>Delete</button>
                                                        <Link className='btn btn-primary text-light' to={`/overview/classes/${items._id}@qwert`}>View</Link>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                ...
                                <>
                                    {/* <img className='search_img' src="https://i.ibb.co/whKdzsX6/White-Grey-and-Blue-Browser-Nerdy-Birthday-Card.png" alt="" /> */}
                                    <img className='search_img' src="https://media.istockphoto.com/id/1266840330/vector/vector-illustration-of-document-search.jpg?s=1024x1024&w=is&k=20&c=f2eU6PRmaypWN39E376fP5S_E6UdBtBnFpTBtajujz4=" alt="" />
                                </>
                                }
                            </div>
                        </div>
                        ...
                        <div className="modal-body">
                            <form action="" onSubmit={updateCls}>
                                ...
                                <div className="mt-3">
                                    ...
                                    <label htmlFor="">teacherId</label>
                                    <input type="text" name='teacherId' disabled value={updateClass?.teacherId} className='form-control' onChange={inputHandler} />
                                </div>}
                                <div className="mt-3">
...
// https://media.istockphoto.com/id/1073831676/vector/employer-with-magnifying-glass-exploring-application-papers.jpg?s=1024x1024&w=is&k=20&c=XSJgv2vRD7OXSrT9yorWbm81J2emCOQaUofk2X8aqiE=
// import React, { useEffect, useState } from 'react'
// import axios from 'axios';
// import { ToastContainer, toast } from 'react-toastify';
// import { Link } from 'react-router-dom';
...
//     const [day, setDay] = useState('off');
//     const [teacher, setTeacher] = useState('off');
...
//     // SHOW TITTLE INPUT FIELD
//     const inputValueByTittle = (event) => {
//         setTittle(tittle === 'on' ? 'off' : 'on')
//         setDay(day === 'on' ? 'off' : '')
//         setTeacher(teacher === 'on' ? 'off' : '')
//         setTime(time == 'on' ? "off" : '')
//     }

//     // SHOW CLASS CODE INPUT FIELD
//     const inputValueByDay = (event) => {
//         setDay(day === 'on' ? 'off' : 'on')
//         setTittle(tittle == 'on' ? "off" : '')
//         setTeacher(teacher === 'on' ? 'off' : '')
//         setTime(time == 'on' ? "off" : '')
//     }

//     // SHOW TEACHER ID INPUT FIELD
//     const inputValueByTeacher = (event) => {
//         setTeacher(teacher === 'on' ? 'off' : 'on')
//         setDay(day === 'on' ? 'off' : '')
//         setTittle(tittle == 'on' ? "off" : '')
//         setTime(time == 'on' ? "off" : '')
//     }

//     // SHOW TIME INPUT FIELD
//     const inputValueByTime = (event) => {
//         setTime(time == 'on' ? "off" : 'on')
//         setDay(day === 'on' ? 'off' : '')
//         setTittle(tittle == 'on' ? "off" : '')
//         setTeacher(teacher === 'on' ? 'off' : '')
//     }
...
//             }
//             if(showTeacherOption.role == "admin"){
//                 var res = await axios.get('http://localhost:4000/api/classes/getall', {
//                     headers: {
//                         "Authorization": "bearer " + localStorage.getItem("tokeId")
//                     }
//                 });
//             }
//             if(showTeacherOption.role == "teacher"){
//                 var res = await axios.get(`http://localhost:4000/api/classes/showClassIncludeTeachers/${showTeacherOption._id}`, {
//                     headers: {
//                         "Authorization": "bearer " + localStorage.getItem("tokeId")
//                     }
//                 });
...
//                             <div className='col-sm-3'>
//                                 <label htmlFor="">Search by Tittle</label>
//                                 <input onChange={inputValueByTittle} type="radio" className='mx-3' name="tittle" id="" />
//                             </div>
//                             <div className="col-sm-3">
//                                 <label htmlFor="">Search by classCode</label>
//                                 <input onChange={inputValueByDay} type="radio" className='mx-3' name="tittle" id="" />
//                             </div>
//                             <div className="col-sm-3">
//                                 <label htmlFor="">Search by Teacher</label>
//                                 <input onChange={inputValueByTeacher} className='mx-2' type="radio" name="tittle" id="" />
//                             </div>
//                             <div className="col-sm-3">
//                                 <label htmlFor="">Search by Time</label>
//                                 <input onChange={inputValueByTime} type="radio" className='mx-3' name="tittle" id="" />
//                             </div>
//                         </div>
//                         <div className='mt-4'>
//                             {tittle == 'on' ? <input type="search" placeholder='search by Class Tittle' onChange={searchByTittle} className='form-control ' /> : ''}
//                             {day == 'on' ? <input type="search" placeholder='search by classCode' onChange={searchByTittle} className='form-control ' /> : ''}
//                             {teacher == 'on' ? <input type="search" placeholder='search by teacherID' onChange={searchByTittle} className='form-control ' /> : ''}
//                             {time == 'on' ? (
//                                 <>
//                                     <div className='d-flex justify-content-between'>
//                                         <div className="col-sm-5">
...
//                                                         <button onClick={() => editClasses(items._id)} className="btn btn-primary" data-bs-toggle="modal" data-bs-target="#staticBackdrop">Edit</button>
//                                                         <button onClick={() => deleteClasses(items._id)} className='btn btn-danger'>Delete</button>
//                                                         <Link className='btn btn-primary text-light' to={`/admin/classes/${items._id}@qwert`}>View</Link>
//                                                     </div>
//                                                 </div>
//                                             </div>
//                                         ))}
